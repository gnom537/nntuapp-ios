...
// MARK: - getMarks()
func getMarks (html: String) -> AllData {
        var TheData = AllData()
        var tempSem = Sem()
    
        if (html == "") {return TheData}
    if (html.contains("не найден")) {return TheData}
...
//MARK: - updateMarkInfo()
func updateMarkInfo(completion: @escaping (AllData, String) -> (Void)) {
